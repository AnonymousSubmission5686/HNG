...
def build_train_transformer(cfg):
    ...
    # resize
    res2.append(T.Resize((cfg.DATA.height, cfg.DATA.width), interpolation=3))

    # horizontal filp
    # if cfg.DATA.TRAIN.is_flip:
    #     res2.append(T.RandomHorizontalFlip(p=cfg.DATA.TRAIN.flip_prob))
    #
    # # padding
    # if cfg.DATA.TRAIN.is_pad:
    #     res2.extend(
    ...
    # totensor
    res2.append(T.ToTensor())

    # normalize
    res2.append(T.Normalize(mean=cfg.DATA.norm_mean, std=cfg.DATA.norm_std))
    ...
    if cfg.DATA.TRAIN.is_mutual_transform:
        return MutualTransform(T.Compose(res), T.Compose(res2), cfg.DATA.TRAIN.mutual_times)
